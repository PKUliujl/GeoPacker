...
def preprocess_singlechain(inputPATH,pdbname, inputfile, chainID, begins_num, ends_num ):
    ...
    for model in structure:
        ...
        for chain in model:
         if chain.id == chainID:  
            ...
            if int(begins_num)==0:
                print("Sequence: ",sequence_list)  #dtype
            ...
            edge_attributes = []
            edge_index =[]
            for i in range(int(begins_num),int(ends_num)): #len(matrix)
                each_node_feature = []
                ...
                NA = ns.search(chain[int(matrix .iloc[i,3])]['CA'].coord, 10)  ##Neighbor Atoms
            ...
            #print( node_feature )
            #np.savetxt( os.path.join(inputPATH, pdbname + '_' + chainID+'_nodecategory'),np.array(node_category) )
            #print( node_category )
            #np.savetxt( os.path.join(inputPATH, pdbname + '_' + chainID+'_edgeattr'),np.array(edge_attributes) )
            #print( edge_attributes )
            #np.savetxt( os.path.join(inputPATH, pdbname + '_' + chainID+'_edgeindex'),np.array(edge_index) )  
            #print ( edge_index )
